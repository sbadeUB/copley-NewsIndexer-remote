...
public class QueryParser
{
	...
	static String[] stopwords={"a","able","about","across","after","all","almost","also","am",
		...
		for(int j=0;j<splitSpace.length;j++)
		{
    		...
    		else//either single word or with multiple words having no operators in between
    		{
    			...
    			else
    			{
    				...
    				boolean isStopWordSet=false;
					String str1=splitSpace[j].toLowerCase();
					if(Arrays.asList(stopwords).contains(str1))
					{
						isStopWordSet=true;
						splitSpace[j]="";
						splitSpace[j-1]="";
						String temp=multiword.toString();
						temp=temp.trim();
						multiword.delete(0, multiword.length()-1);
						String[] x=temp.split(" ");
						for(int i=0;i<x.length-1;i++)
							multiword.append(x[i]+" ");
						countOperators=countOperators-1;
	...
	public static String getFinalParsedQuery()
	{
		...
		{
			isError=true;
			System.out.println("eror infiltering");
		}
			...
			while(!splitSpace[j].endsWith(")") && (!generalOperatorStack.isEmpty()))
			{
				...
				else
				{
					...
					else
					{
						...
						{
						multiword.append("* ");
						ConsumeString=ConsumeString+" "+splitSpace[j].trim();
						generalOperandStack.push(splitSpace[j].trim());
						}
						else
						{
							isStopWordSet=false;
